    ...
    <table>
      ...
      <tbody>
        <tr v-for="meeting in meetings" :key="meeting.id">
          <td>
            <input v-if="editable && meeting.role === 'Organizer'" type="text" v-model="meeting.title">
            <span v-else>{{ meeting.title }}</span>
          </td>
          <td>
            <textarea v-if="editable && meeting.role === 'Organizer'" v-model="meeting.description"></textarea>
            <span v-else>{{ meeting.description }}</span>
          </td>
          <td>
            <input v-if="editable && meeting.role === 'Organizer'" type="datetime-local" v-model="meeting.time">
            <span v-else>{{ meeting.time }}</span>
          </td>
          <td>
            <input v-if="editable && meeting.role === 'Organizer'" type="text" v-model="meeting.location">
            <span v-else>{{ meeting.location }}</span>
          </td>
          <td>{{ meeting.role }}</td>
          <td>
            <button v-if="editable" @click="deleteMeeting(meeting)">Delete</button>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  setup() {
    ...
    const deleteMeeting = async meeting => {
      try {
        const response = await api.deleteMeeting(meeting.meetingid);
        if (response.data.success) {
          meetings.value = meetings.value.filter(m => m.id !== meeting.id);
        } else {
          errorMessage.value = response.data.message || 'Failed to delete meeting';
        }
      } catch (error) {
        console.error('删除会议失败', error);
        errorMessage.value = 'Error while deleting meeting';
      }
    };
